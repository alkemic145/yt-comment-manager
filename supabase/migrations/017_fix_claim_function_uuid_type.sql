-- 1. Drop existing function overloads cleanly
...
drop function if exists public.claim_pending_comment_automation_jobs(text, int, int);
drop function if exists public.claim_pending_comment_automation_jobs(uuid, int, int);
drop function if exists public.claim_pending_comment_automation_jobs;

-- 2. Create the hardened function with UUID parameter
