-- Keep automatic replies focused on recent comments. Older unreplied
-- comments should not be picked up when the scheduler catches up.
  ...
  with candidates as (
    ...
    where c.user_id = p_user_id
      ...
      and c.published_at >= now() - (
        greatest(coalesce(p_max_age_hours, 24), 1) * interval '1 hour'
      )
      and (
...
revoke execute on function public.claim_pending_comment_automation_jobs(uuid, integer, integer, integer)
  from public, anon, authenticated;

grant execute on function public.claim_pending_comment_automation_jobs(uuid, integer, integer, integer)
  to service_role;
